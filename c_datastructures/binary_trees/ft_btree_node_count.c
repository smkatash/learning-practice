...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_btree_node_count.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: kanykei <kanykei@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/26 19:49:40 by ktashbae          #+#    #+#             */
/*   Updated: 2022/09/15 00:45:53 by kanykei          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Each node if next left/right node is not found returns +1.
Previous node receives from its successor nodes, left = 1 or right = 1,
adds + 1 to the left/right and summarizes both nodes incl. itself.
Returns +3 to the previous node, continues until the root receives 
final count of nodes.
*/
int	ft_btree_node_count(t_btree *root)
{
	int	left;
	...
	right = 0;
	if (root->left)
		left = ft_btree_node_count(root->left);
	if (root->right)
		right = ft_btree_node_count(root->right);
	if (left > right)
		left = left + 1;
	else
		right = right + 1;
...
// 	ft_print_btree(&root);
// 	printf("\n----------------------\n");
// 	nodes = ft_btree_node_count(root);
// 	printf("Nodes: %d\n", nodes);
// }